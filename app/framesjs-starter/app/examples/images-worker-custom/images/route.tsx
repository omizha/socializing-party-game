...
export const GET = imagesRoute(async (jsx) => {
  const regularFont = fs.readFile(path.join(path.resolve(process.cwd(), 'public'), 'Inter-Regular.ttf'));

  const boldFont = fs.readFile(path.join(path.resolve(process.cwd(), 'public'), 'Inter-Bold.ttf'));
...
function Scaffold({ children }: { children: React.ReactNode }) {
  return (
    <div tw="flex items-stretch relative w-full h-screen bg-white overflow-hidden">
      <div tw="flex flex-col justify-center items-center text-black w-full overflow-hidden">{children}</div>
    </div>
  );
}
