...
export class UserRepository {
  ...
  async deleteOne(filter: FilterQuery<User>, options?: QueryOptions<User>): Promise<boolean> {
    return !!(await this.userModel.deleteOne(filter, options));
  }

  async deleteMany(filter: FilterQuery<User>, options?: QueryOptions<User>): Promise<boolean> {
  ...
  async initializeUsers(stockId: string, options?: QueryOptions<User>): Promise<boolean> {
    try {
